...
const Table = ({ data, sizesData, parametres }) => {
    ...
    useEffect(() => {
        ...
        if (size == 5) {
            ...
            setMinRoomWidth(prev => prev = 4.50)
            setMinRoomHeight(prev => prev = 3.70)
        }
        if (size == 6) {

            setMinWidth(prev => prev = 1.80)
            setMinHeight(prev => prev = 0.90)

            setMinRoomWidth(prev => prev = 4.90)
            setMinRoomHeight(prev => prev = 4.00)
        }

        if (size == 7) {

            setMinWidth(prev => prev = 1.90)
            setMinHeight(prev => prev = 0.95)

            setMinRoomWidth(prev => prev = 5.00)
        ...
        if (size == 11) {
            ...
            setMinWidth(prev => prev = 3.20)
            setMinHeight(prev => prev = 1.60)

            setMinRoomWidth(prev => prev = 6.70)
            setMinRoomHeight(prev => prev = 5.10)

        }
        if (size == 12) {


            setMinWidth(prev => prev = 3.50)
            setMinHeight(prev => prev = 1.75)

            setMinRoomWidth(prev => prev = 7.00)
    ...
    return (
        <div className="main float-right width772" id="bx_117848907_36064" style={{ paddingTop: "36px" }}>
            <span className="h2">
                Бильярдный стол High-style            </span>
            ...
            </p><div className="float-right img-right">


                <a href="https://www.fabrika-start.ru/catalog/sect/176/good/high_style/" rel="nofollow" title="Перейти на страницу товара">
